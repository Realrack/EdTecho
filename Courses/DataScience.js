...
const DataScience = ({ navigation }) => {
  press = (item) =>{
    if(item.id == 1){
      navigation.navigate('sql')
    }
    else if (item.id == 2){
      navigation.navigate('julia')
    }
    else if (item.id == 3){
      navigation.navigate('cplus')
    }
    else if (item.id == 4){
      navigation.navigate('scala')
    }
    else{}
      ...
      <View style={styles.flatlistContainer}>
        <FlatList
          ...
          data={[
            {
              id: 1,
              image: require("../Photos/sql.png"),
              name: "SQL",
            },
            {
              id: 2,
              image: require("../Photos/julia.png"),
              name: "Julia",
            },
            {
              id: 3,
              image: require("../Photos/cpluss.png"),
              name: "C++",
            },
            {
              id: 4,
              image: require("../Photos/scala.png"),
              name: "Scala",
            },
          ]}
          renderItem={({ item }) => {
